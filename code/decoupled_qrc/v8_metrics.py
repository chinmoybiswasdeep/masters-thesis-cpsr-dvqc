...
def _fit_predict(x_train, y_train, x_test, readout):
    ...
    if readout == "whitened_ols":
        ...
        keep = singular > singular[0] * 1e-10 if singular.size else np.zeros(0, dtype=bool)
        whiten_train = u[:, keep]
...
def family_columns(names, family):
    ...
    if family == "current_quadratic_x_delayed_linear":
        return _columns(names, ("J:current_p2_",))
    ...
    if family == "current_quartic_x_delayed_linear":
        return _columns(names, ("J:current_p2_", "J:current_p4_"))
    if family == "delayed_linear_x_delayed_linear":
        return _columns(names, ("J:linear_pair:",))
    if family == "delayed_nonlinear_x_delayed_linear":
        return _columns(names, ("M:", "J:past_p2_"))
...
def geometry(matrix):
    ...
    return {"singular_values": singular.tolist(), "effective_rank": effective_rank, "condition_number": condition}
